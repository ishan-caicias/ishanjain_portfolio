...
| --- | ------------------------------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1  | Desktop WebGPU: 60 fps sustained, startup ≤ 2.5 s | `?engine=babylon&perf=1`, badge `FULL`                            | ☒ **STILL FAILS, 3 rounds** (TR-068 40fps → [TR-083](../test-reports/TR-083.md) 45-52fps, 2026-07-22): startup 762ms ✓ (well under), but fps still short of the 60fps target (75-87% of it this round, up from 67%). Backend not confirmed on any round — this reading may not even be testing the WebGPU path (see TR-083 standing gap).                                                                                                                                                                                                                                                                                   |
| A2  | iPad WebGPU: 60 fps, startup ≤ 3.0 s              | same, cold load                                                   | ☐ still not tested — no iPad available (TR-068, TR-069, [TR-083](../test-reports/TR-083.md))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| A3  | Mid Android WebGPU: ≥ 40 fps, startup ≤ 4.0 s     | same, cold load (TR-043 flagged prior 267 ms reading implausible) | ☑ **PASSES both devices, round 3** (TR-068 + TR-069 + [TR-083](../test-reports/TR-083.md), 2026-07-22): Galaxy S20 FE passes all 3 rounds (45/49, 42, then 45 fps). Galaxy S24 Ultra (flagship) **fails rounds 1-2** (28/35, 32-37 fps) **then PASSES round 3** (60 fps, its best reading by far) — the flagship-underperforms-mid-tier anomaly is REVERSED this round. Cause still unconfirmed: the overlay ENGINE/TIER/device-signature line has now gone unreported for a third consecutive round (TR-083) — the DPR/tier/backend hypothesis remains open, but the symptom it was meant to explain is no longer present. |
| A4  | WebGL2 fallback: ≥ 30 fps, startup ≤ 4.0 s        | force-disable WebGPU or use a non-WebGPU browser                  | ☐ still not tested (TR-068, TR-069, [TR-083](../test-reports/TR-083.md))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
5. ☐ Deploy; validate live headers (securityheaders.com) + smoke the five device classes — **still
   ...
   2026-07-20) and 3 of 5 classes were measured this way, across **3 rounds** now — see TR-068,
   [TR-069](../test-reports/TR-069.md), and [TR-083](../test-reports/TR-083.md) (PF-11 D0.3,
   2026-07-22): round 3 reversed the flagship-underperforms-mid-tier anomaly (A3 now passes both
   devices) though desktop (A1) still misses its 60fps target and the backend/tier confirmation
   remains uncaptured across all 3 rounds — improved, not a clean pass, so this row stays open.
   iPad (deferred/unavailable every round) and WebGL2-fallback classes remain untested; live
   deploy still not done.
