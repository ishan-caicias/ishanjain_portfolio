...
| 4   | #6 card fallback                  | **P1**   | Make the fallback class-aware (stop drawing galaxies as suns); fix the duplicate-id twins; fix the lying caption                                                                                                              | Content decision on real imagery     |
| 5   | §1b backend invisibility          | **P1**   | Add `backend` to `PerfSnapshot`; log the fallback once                                                                                                                                                                        | —                                    |
| 6   | #7a Sun sphere                    | **P2**   | Add sun to the planet-texture pipeline                                                                                                                                                                                        | Licensed source texture              |
| 7   | #7b white dwarf body              | **P2**   | Either give `fs-` class-row arrivals a real body, or stop offering them as destinations                                                                                                                                       | Design decision                      |
| 8   | #5 warp settle                    | **P3**   | Restore the spec's 4.3 waypoint or re-brief                                                                                                                                                                                   | Vega MOTION-AUDIT                    |
| 9   | #2 gaia-tiny chunk value          | **P3**   | Measure whether chunks 4–8 are visually distinguishable                                                                                                                                                                       | Owner eyes                           |

**Both P0 items are closed ([TR-115](../test-reports/TR-115.md), 905 unit · 177 E2E, zero failures). Items 3-9 remain open.**
