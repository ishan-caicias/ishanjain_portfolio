...
> **⚠ Audit correction (2026-07-17):** the "Current State" section describes `main`, not the
> working branch (which is Astro 7 / TS 6 with the space scene already shipped). The
> **"Validated delivery decision"** section below describes an implementation that was **rolled
> back** — its phase-report links dangle, no runtime GLB exists, and `three` is not a
> dependency. The Options A/B/C analysis itself remains valid; the refined recommendation
> (in-engine GLB renderer first, Three.js hybrid as gated fallback) is in
> [2026-07-17-codex-analysis-audit.md](2026-07-17-codex-analysis-audit.md).
