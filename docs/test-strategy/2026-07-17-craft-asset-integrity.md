...
| Unit/integration (vitest) | `tests/unit/craft-loader.test.ts`   | (Added at P1) The consumer side of the contract: `parseCraftGlb` against the real committed GLBs — meshopt decode, quantized-attribute shapes, material role resolution, WebP payload magic, and an independent re-verification that the model matrix maps the mesh into the wireframe-parity unit box |
| Unit (vitest)             | `tests/unit/ship-dynamics.test.ts`  | (Added at P2) Flight math: spring convergence/overshoot/critical-damping/dt-stability, NDC↔view round-trip through the engine's projection convention, and the legacy screen-composition parity proofs                                                                                                 |
...
| E2E (Playwright)          | `tests/e2e/craft-ship.spec.ts`      | (Added at P1) Full runtime path: `?craft=1k\|2k` reaches `data-craft-state="ready"` with no page/console errors; without the flag the craft path never engages                                                                                                                                         |
