...
| `data/build/*.js` (streaming parsers for the raw Gaia Sky packs) | 247 lines + missing exec body in `03-curated.js` | **Deferred, likely not needed.** Per `DATA.md`, every runtime asset is already derived and committed — the raw `gaia_datasets/` pipeline is only needed to *regenerate or extend* the dataset (e.g. the full DR3 octree upgrade `FULL-DATASET-PIPELINE.md` describes). Not required to ship the site as designed. |
...
- Temporary verification route `src/pages/space-test.astro` (bare page, no BaseLayout/header/footer) —
  ...
  into `index.astro`. *(This was missed at the time - it stayed on disk through Phases 2-5, building
  into `dist/space-test/` and appearing in the production sitemap with no `noindex`/robots exclusion.
  Caught and removed during a full delivery-plan audit after Phase 6 - see §5.)*
...
mission-control search can travel *to* a station even without the visual sprite markers.
...
**not** ported - it's cosmetically separate from the "Moon Base" travel *station*, which already
...
*(Later retired in Phase 6, once the user explicitly directed that phase to completion - see below.)*
...
screen coordinates. Station sprite *visual* positioning could only be verified by tracing the
