...
test.describe("Space Scene", () => {
  ...
  test("RANDOM JUMP mission control button travels somewhere", async ({
    page,
  }) => {
    // PF-11 D1.2 named test change (CLAUDE.md #15): the mission control bar (and its
    // random-jump button) is hidden under body.ij-loading until a real visitor action,
    // same as the hero copy — dismiss the PRE-FLIGHT gate first, as a real visitor would
    // before ever seeing this button. Assertion below unchanged.
    //
